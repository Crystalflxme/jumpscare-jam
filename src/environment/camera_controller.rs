...
fn run_camera_controller(
    ...
    if let Ok((mut transform, mut controller)) = query.get_single_mut() {
        ...
        for ev in evr_mouse_wheel.read() {
        ...
        for ev in evr_mouse_motion.read() {
            if ev.delta == Vec2::ZERO || !cursor_grab {
                continue;
            }

            // Apply look update
            controller.pitch = (controller.pitch
                - ev.delta.y * RADIANS_PER_DOT * controller.sensitivity)
                .clamp(-PI / 2., PI / 2.);
            controller.yaw -=
                ev.delta.x * RADIANS_PER_DOT * controller.sensitivity;
            transform.rotation =
